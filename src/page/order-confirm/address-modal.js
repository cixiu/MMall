/*
* @Author: cixiu
* @Date:   2017-07-27 19:30:11
* @Last Modified by:   cixiu
* @Last Modified time: 2017-07-28 09:44:40
*/
...
var addressModal = {
	...
	loadModal: function () {
		var addressModaHtml = _mm.renderHtml(templateAddressModal, {
			isUpdate: this.option.isUpdate,
			data: this.option.data
		});
		this.$modalWrap.html(addressModaHtml);
		// 加载省份
		this.loadProvince();
	},
	bindEvent: function () {
		...
		this.$modalWrap.find('.address-btn').click(function () {
			...
			} else if (isUpdate && receiverInfo.status) {  // 更新地址，且验证通过
		...
		// 点击modal-container时不关闭弹窗，需要阻止冒泡
	...
	getReceiverInfo: function () {
		...
		if (!receiverInfo.receiverName) {
			...
			result.errMsg = '请输入详细地址信息';
		} else if (!receiverInfo.receiverPhone) {
			result.errMsg = '请输入收货人手机号码';
		} else {
			result.status = true;
